...
%
%   [x1Array, x2Array] = computeIsormContour(PM); 
%
% Or use custom parameters and plot the contour in U- and X-space:
%
%   computeIsormContour(PM, 4, 15, 1);
%
...
function [x1Array, x2Array] = computeIsormContour(PM, alpha, ...
    nAngles, shouldPlot)
...
if ~strcmp(PM.modelType, 'CMA')
    disp('Stopping computeIsormContour as the input model is not of type CMA');
    x1Array = nan(1);
    x2Array = nan(1);
    return
end
